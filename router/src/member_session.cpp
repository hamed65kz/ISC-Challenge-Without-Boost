...
/**
 * @brief Starts the member session by initiating an asynchronous read
 * operation.
 *
 * This method begins reading a handshake message from the member's socket.
 * Once the message is received, it calls the handle_handshake method to process
 * it.
 *
 * @note This method is intended to be called when a new connection is
 * established.
 */
void MemberSession::start() {
  boost::asio::async_read(
      socket_, boost::asio::buffer(id_buffer_, 3),
      boost::asio::bind_executor(
          strand_,
          [self = shared_from_this()](boost::system::error_code ec, size_t) {
            if (!ec) self->handle_handshake();
          }));
}
 ...
 * It checks if the maximum number of members is reached and logs an error if
 * so. If the member ID already exists, it logs a warning and removes the
 * previous session. Finally, it adds the member to the members map and starts
 ...
 *
 * This method sets up an asynchronous read operation to receive messages from
 * the member. Upon receiving a message, it calls the process_message method to
 * handle it. If an error occurs during reading, it logs an error message.
 */
void MemberSession::start_reading() {
  async_read(
      socket_, buffer(message_buffer_, MESSAGE_LENGTH),
      boost::asio::bind_executor(
          strand_,  // Add strand protection
          [self = shared_from_this()](boost::system::error_code ec, size_t) {
            if (!ec) {
...
/**
 * @brief Processes the received message and forwards it to the appropriate
 * destination.
 *
 * This method extracts the destination ID from the message and checks if the
 * destination member exists in the members map. If found, it forwards the
 * message to that member. If the destination is not found, it logs an error
 * message.
 */
void MemberSession::process_message() {
  ...
  std::shared_ptr<MemberSession> dst_session;
  {
  ...
  if (dst_session) {
    dst_session->forward_message(message);
 ...
 *
 * This method sends the specified message to the member's socket using an
 * asynchronous write operation. If an error occurs during the sending process,
...
void MemberSession::forward_message(const std::string& message) {
  ...
  boost::asio::async_write(
      ...
      boost::asio::bind_executor(
          strand_, [self = shared_from_this(), message_ptr](auto ec, auto) {
            if (ec) LOG_ERROR("Send error: {}", ec.message());
          }));
}
